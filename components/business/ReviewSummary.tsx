...
export function ReviewSummary({ breakdown, isEstimated = false, className }: ReviewSummaryProps) {
  ...
  return (
    <div className={cn('rounded-card border border-ink-200 bg-ink-50 p-5', className)}>
      <div className="grid gap-6 sm:grid-cols-[auto_1fr]">
        <div className="flex flex-col items-center justify-center gap-1 sm:pr-6 sm:border-r sm:border-ink-200">
          <p className="text-5xl font-medium tabular-nums text-ink-900">{formatRating(average)}</p>
          ...
          {STARS.map((star) => {
            ...
            return (
              <li key={star} className="flex items-center gap-3 text-sm">
                ...
                <span
                  className="h-2 flex-1 overflow-hidden rounded-full bg-ink-150"
                  role="img"
      ...
      {isEstimated && (
        <p className="mt-4 border-t border-ink-200 pt-3 text-xs text-ink-400">
          Répartition estimée à partir de la note moyenne et du nombre total d’avis : la source ne fournit pas
          le détail par étoile.
        </p>
      )}
    </div>
  )
